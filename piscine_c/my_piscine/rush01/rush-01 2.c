...
/*   Updated: 2019/06/16 20:09:21 by zexu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			compare_lib(int *lib, int *src, int count, int *tab_params, int params_index)
{
	...
	for(int i = 0; i < count; i++)
	{
		...
		for(int j = 0; j < 4; j++)
		{
			...
			{
				flag = 0;
				if( *(src + i * 16 + 4*j + 0) == *(lib + 4 * (position(*(tab_params + params_index)) - 1 + l) + 0))
					flag++;	
				if( *(src + i * 16 + 4*j + 1) == *(lib + 4 *  (position(*(tab_params + params_index)) -1 + l) + 1))
					flag++;	
				if( *(src + i * 16 + 4*j + 2) == *(lib + 4*  (position(*(tab_params + params_index)) - 1 + l) + 2))
					flag++;	
				if( *(src + i * 16 + 4*j + 3) == *(lib + 4*  (position(*(tab_params + params_index)) -1  + l) + 3))
					flag++;	
				if(flag == 4)
				{
					printf("origin : %d,  with : %d.  i : %d, j : %d, l: %d \n", *(src + i * 16 + 4 * j + 0) , *((lib +4* (position( *(tab_params + params_index)) -1+ l)+ 0), i, j, l );
					printf("origin : %d,  with : %d.  i : %d, j : %d, l: %d \n", *(src + i * 16 + 4 * j + 1) , *((lib +4* (position( *(tab_params + params_index))- 1+ l)+ 1), i, j, l );
					printf("origin : %d,  with : %d.  i : %d, j : %d, l: %d \n", *(src + i * 16 + 4 * j + 2) , *((lib +4* (position( *(tab_params + params_index)) -1+ l)+ 2), i, j, l );
					printf("origin : %d,  with : %d.  i : %d, j : %d, l: %d \n", *(src + i * 16 + 4 * j + 3) , *((lib +4* (position( *(tab_params + params_index)) - 1+ l)+ 3), i, j, l );
					flag_general++;		
					break;
				}
			}

			for(int l = 0; l < show_possibility(*(tab_params + params_index + 1)); l++)
			{
				flag = 0;
				if( *(src + i * 16 + 4*j + 0) == *(lib + (position(*(tab_params + params_index + 1)) + l) + 0))
					flag++;	
				if( *(src + i * 16 + 4*j + 1) == *(lib + (position(*(tab_params + params_index + 1)) + l) + 1))
					flag++;	
				if( *(src + i * 16 + 4*j + 2) == *(lib + (position(*(tab_params + params_index + 1)) + l) + 2))
					flag++;	
				if( *(src + i * 16 + 4*j + 3) == *(lib + (position(*(tab_params + params_index + 1)) + l) + 3))
					flag++;	
			...
			for(int l = 0; l < show_possibility(*(tab_params + params_index + 2)); l++)
			{
				flag = 0;
				if( *(src + i * 16 + 4*j + 0) == *(lib + (position(*(tab_params + params_index + 2)) + l) + 0))
					flag++;	
				if( *(src + i * 16 + 4*j + 1) == *(lib + (position(*(tab_params + params_index + 2)) + l) + 1))
					flag++;	
				if( *(src + i * 16 + 4*j + 2) == *(lib + (position(*(tab_params + params_index + 2)) + l) + 2))
					flag++;	
				if( *(src + i * 16 + 4*j + 3) == *(lib + (position(*(tab_params + params_index + 2)) + l) + 3))
					flag++;	
			...
			for(int l = 0; l < show_possibility(*(tab_params + params_index + 3)); l++)
			{

				flag =0;
				if( *(src + i * 16 + 4*j + 0) == *(lib + (position(*(tab_params + params_index + 3)) + l) + 0))
					flag++;	
				if( *(src + i * 16 + 4*j + 1) == *(lib + (position(*(tab_params + params_index + 3)) + l) + 1))
					flag++;	
				if( *(src + i * 16 + 4*j + 2) == *(lib + (position(*(tab_params + params_index + 3)) + l) + 2))
					flag++;	
				if( *(src + i * 16 + 4*j + 3) == *(lib + (position(*(tab_params + params_index + 3)) + l) + 3))
					flag++;	
...
int			selection_left(int *all, int count, int *lib, int * tab_params)
{
	...
	compare_lib(lib, new, new_count, tab_params, 4);
...
int main(int argc, char *argv[])
{
	...
	count = selection_left(all, count, p_lib, tab_params);
	/*
