...
int				pf_signed_int(va_list argp, t_pf **list)
{
	char		*str;
	t_pf		*new;
...
int				pf_unsigned_int(va_list argp, t_pf **list)
{
	char		*str;
	t_pf		*new;
...
int				pf_hex(va_list argp, t_pf **list, int is_maj)
{
	char		*str;
	t_pf		*new;
	char		type;
	...
	if (is_maj)
		...
		type = 'x';	
	if (!(new = pf_output_new(str, type)))
		return (-1);
	return (pf_output_add(list, new));
}

int				pf_pointer(va_list argp, t_pf **list)
{
	char		*str;
	t_pf		*new;
